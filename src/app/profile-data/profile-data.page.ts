...
export class ProfileDataPage implements OnInit {
  ...
  async ngOnInit() {
    await Preferences.get({ key: 'loginData' }).then((storedUser) => {
      if (storedUser && storedUser.value) {
        this.userdata = storedUser ? JSON.parse(storedUser.value) : null;
        this.load_circle_Data();

        if (this.userdata?.data[0]?.designation_name === 'BG') {
        ...
        } else if (this.userdata?.data[0]?.designation_name === 'RO') {
          this.autoCompleteFields = [
            {
              label: this.translate.instant('profile-setup.s-circle'),
              controlName: 'circleControl',
        ...
        } else if (this.userdata?.data[0]?.designation_name == 'SDO') {
          this.autoCompleteFields = [
            {
              label: this.translate.instant('profile-setup.s-circle'),
              controlName: 'circleControl',
        ...
        } else if (this.userdata.data[0]?.designation_name == 'DFO') {
          this.autoCompleteFields = [
            {
              label: this.translate.instant('profile-setup.s-circle'),
              controlName: 'circleControl',
        ...
        } else if (this.userdata.data[0]?.designation_name == 'CFO') {
          this.autoCompleteFields = [
            {
              label: this.translate.instant('profile-setup.s-circle'),
              controlName: 'circleControl',
    ...
    this.circleControl.valueChanges.subscribe((selectedCircle) => {
      if (
        ...
        {
          this.userdata.data[0]?.designation_name != 'CFO' &&
            this.divisionControl.enable();
        }
      } else {
    ...
    this.divisionControl.valueChanges.subscribe((selectedDivision) => {
      if (
        ...
        {
          this.userdata.data[0]?.designation_name != 'DFO' &&
            this.subDivisionControl.enable();
        }
      } else {
        this.subDivisionControl.reset();
        this.subDivisionControl.disable();
    ...
    this.subDivisionControl.valueChanges.subscribe((selectedSubDivision) => {
      if (
        ...
        {
          this.userdata.data[0]?.designation_name != 'SDO' &&
    ...
    this.rangeControl.valueChanges.subscribe((selectedRange) => {
      if (
        ...
        {
          this.userdata.data[0]?.designation_name != 'RO' &&
            this.beatControl.enable();
        }
      } else {
        this.beatControl.reset();
        this.beatControl.disable();
  ...
  async onClick() {
    if (
      typeof this.circleControl.value !== 'object' ||
      !this.circleControl.value?.id
    ) {
      alert('Please select a valid Circle from dropdown');
      return;
    }
    if (
      this.divisionControl.enabled &&
      (typeof this.divisionControl.value !== 'object' ||
        !this.divisionControl.value?.id)
    ) {
      alert('Please select a valid Division from dropdown');
      return;
    }
    if (
      this.subDivisionControl.enabled &&
      (typeof this.subDivisionControl.value !== 'object' ||
        !this.subDivisionControl.value?.id)
    ) {
      alert('Please select a valid Sub Division from dropdown');
      return;
    }
    if (
      this.rangeControl.enabled &&
      (typeof this.rangeControl.value !== 'object' ||
        !this.rangeControl.value?.id)
    ) {
      alert('Please select a valid Range from dropdown');
      return;
    }
    if (
      this.beatControl.enabled &&
      (typeof this.beatControl.value !== 'object' ||
        !this.beatControl.value?.id)
    ) {
      alert('Please select a valid Beat from dropdown');
      return;
    }
    let data = {
      empId: this.userdata.data[0]?.emp_id + '',
